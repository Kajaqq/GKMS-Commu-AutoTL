...
- `config.py`: model, folder, Excel header, and wrapping constants.
- `prompts.py`, `character_styles.py`, `dictionary.py`: translation prompt data and static term/style mappings.
- `formatting.py`, `text_utils.py`: output cleanup and layout helpers.
- `IN/`: input `.xlsx` files. `OUT/`: generated translated `.xlsx` files.
- `Architecture.md`: higher-level dataflow and module overview.
