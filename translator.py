...
class GeminiTranslationClient:
    def __init__(
        ...
        self._request_limiter = (
            ...
            if self._rate_limit_config.requests_per_minute > 0
            else None
        )
        self._input_token_limiter = (
            TokenBucketRateLimiter(self._rate_limit_config.input_tokens_per_minute)
            if self._rate_limit_config.input_tokens_per_minute > 0
            else None
        )
        self._daily_request_limiter = (
            DailyRequestLimiter(self._rate_limit_config.requests_per_day)
            if self._rate_limit_config.requests_per_day > 0
            else None
        )
    ...
    def translate_batch(self, batch_prompt) -> str:
        ...
        estimated_input_tokens = _estimate_tokens(
            prompt_text,
            self._rate_limit_config.token_estimate_chars_per_token,
        )
