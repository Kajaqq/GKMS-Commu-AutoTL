...
- `ai/translator.py`: Gemini client setup and API calls.
...
- `config/config.py`: model, folder, fixed Excel header, wrapping, safety, and Gemini generation constants.
...
- `Architecture.md`: current architecture notes, dataflow, extension points, and important invariants.

There is currently no dedicated `tests/` directory.

## Architecture & Dataflow Notes

The current architecture is documented in `Architecture.md`. Read it before changing workbook flow, Gemini integration, prompt construction, response validation, or formatting behavior.
...
6. `GeminiTranslationClient.translate_batch()` uses `google-genai`, local token/request/day limiters, configured retry options, and JSON structured output.
...
The project uses the modern `google-genai` SDK. Do not replace it with legacy Gemini/Vertex SDKs. `ai/translator.py` uses `AI_STUDIO_API_KEY` when present and otherwise attempts an enterprise client that depends on Google Cloud/Agent Platform credentials in the environment.
...
Do not commit real API credentials. Use `.env` for `AI_STUDIO_API_KEY`; enterprise/Google Cloud credentials should come from the surrounding environment rather than committed files. `.env.sample` documents the API-key variable currently expected by the repo. Treat generated `OUT/` files as review artifacts, not source-of-truth configuration.
